let clothes = [
  {
      type:"top",
      quantity:1,
      size:"M",
      img1: "https://m.media-amazon.com/images/I/818m3AOjXCL._AC_UL800_FMwebp_QL65_.jpg",
      price: "Rs. 399",
      cost: 399,
      selling_cost: 999,
      sellPrice: "Rs. 999",
      discount: "(60% OFF)",
      ratings: "4.1",
      reviews: "14.1k",
      brand: "usp",
      name: "Yellow Classic",
      color: "yellow",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/61rmlqc8ekL._AC_UL800_QL65_.jpg",
      price: "Rs. 499",
      cost: 499,
      selling_cost: 999,
      sellPrice: "Rs. 999",
      discount: "(52% OFF)",
      ratings: "4.3",
      reviews: "1.1k",
      name: "Teal Texas",
      color: "green",
      brand: "nike",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/71xlUtvITyL._AC_UL800_QL65_.jpg",

      price: "Rs. 599",
      cost: 599,
      selling_cost: 999,
      sellPrice: "Rs. 999",
      discount: "(52% OFF)",
      ratings: "4.3",
      reviews: "241",
      name: "Black Diamond",
      color: "black",
      brand: "adidas",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/81wrZBgXEkL._AC_UL800_FMwebp_QL65_.jpg",
      price: "Rs. 843",
      cost: 843,
      selling_cost: 1284,
      sellPrice: "Rs. 1284",
      discount: "(32% OFF)",
      ratings: "4.6",
      reviews: "12k",
      name: "Volcanic Red",
      color: "red",
      brand: "puma",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/61wdVJfjUWL._AC_UL800_FMwebp_QL65_.jpg",
      price: "Rs. 843",
      selling_cost: 872,
      sellPrice: "Rs. 872",
      cost: 872,
      discount: "(15% OFF)",
      ratings: "4.5",
      reviews: "3k",
      name: "Creamy Simpson",
      color: "yellow",
      brand: "tantra",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/91EgMLmGswL._AC_UL800_QL65_.jpg",
      price: "Rs. 1139",
      cost: 1139,
      selling_cost: 1874,
      sellPrice: "Rs. 1874",
      discount: "(47% OFF)",
      ratings: "4.5",
      reviews: "5.2k",
      name: "Inspire Edition",
      brand: "tantra",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/71VVfOV7s2L._AC_UL800_QL65_.jpg",
      price: "Rs. 1379",
      cost: 1379,
      selling_cost: 2299,
      sellPrice: "Rs. 2299",
      discount: "(40% OFF)",
      ratings: "4.1",
      reviews: "2.6k",
      name: "Gentle Giants",
      color: "white",
      brand: "allen solly",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/51McZFUEPNL._AC_UL800_QL65_.jpg",
      price: "Rs. 1523",
      cost: 1523,
      selling_cost: 2031,
      sellPrice: "Rs. 2031",
      discount: "(70% OFF)",
      ratings: "4.3",
      reviews: "11.3k",
      name: "Techy Yooky",
      color: "white",
      brand: "allen solly",
    },
  {
    
    type:"top",
    quantity:1,
    size:"M",  
    img1: "https://m.media-amazon.com/images/I/61Jayv1qo6L._AC_UL800_QL65_.jpg",
      price: "Rs. 1499",
      cost: 1499,
      selling_cost: 1591,
      sellPrice: "Rs. 1591",
      discount: "(3% OFF)",
      ratings: "4.1",
      reviews: "7.9k",
      name: "Blue Whalsic",
      color: "black",
      brand: "nike",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/81DOBXFFOGL._AC_UL800_QL65_.jpg",
      price: "Rs. 1099",
      cost: 1099,
      selling_cost: 1991,
      sellPrice: "Rs. 1991",
      discount: "(31% OFF)",
      ratings: "4.3",
      reviews: "987",
      name: "Stripes leader",
      color: "blue",
      brand: "puma",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/71k858XN9dL._AC_UL800_QL65_.jpg",
      price: "Rs. 279",
      cost: 279,
      selling_cost: 291,
      sellPrice: "Rs. 291",
      discount: "(2% OFF)",
      ratings: "3.1",
      reviews: "447",
      name: "The Eight",
      color: "blue",
      brand: "adidas",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/810XLWLRufL._AC_UL800_QL65_.jpg",
      price: "Rs. 359",
      cost: 359,
      selling_cost: 399,
      sellPrice: "Rs. 399",
      discount: "(10% OFF)",
      ratings: "4.1",
      reviews: "3k",
      name: "Retro Master",
      color: "black",
      brand: "nike",
    },


  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/71+KW+fb1oL._AC_UL800_QL65_.jpg",
      price: "Rs. 769",
      cost: 769,
      selling_cost: 899,
      sellPrice: "Rs. 899",
      discount: "(23% OFF)",
      ratings: "4.3",
      reviews: "8.9k",
      name: "Royal White",
      color: "white",
      brand: "adidas",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/91BugKTBWDL._AC_UL800_QL65_.jpg",
      price: "Rs. 567",
      cost: 567,
      selling_cost: 599,
      sellPrice: "Rs. 599",
      discount: "(2% OFF)",
      ratings: "3.8",
      reviews: "9.6k",
      color: "pink",
      name: "Axel Z",
      brand: "allen solly",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/71A2p4zM9KL._AC_UL800_QL65_.jpg",
      price: "Rs. 599",
      cost: 599,
      selling_cost: 699,
      sellPrice: "Rs. 699",
      discount: "(12% OFF)",
      ratings: "4.2",
      reviews: "2k",
      name: "GYM Tech",
      color: "white",
      brand: "nike",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/813YzCT7qNL._AC_UL800_QL65_.jpg",
      price: "Rs. 479",
      cost: 479,
      selling_cost: 849,
      sellPrice: "Rs. 849",
      discount: "(37% OFF)",
      ratings: "4.6",
      reviews: "37k",
      name: "Athleti-X",
      color: "blue",
      brand: "adidas",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/714Sc6sKBfL._AC_UL800_QL65_.jpg",
      price: "Rs. 579",
      cost: 579,
      selling_cost: 749,
      sellPrice: "Rs. 749",
      discount: "(47% OFF)",
      ratings: "3.8",
      reviews: "5.1k",
      name: "Dawn Bluem",
      color: "black",
      brand: "reebok",
    },
  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/81FC0ECOW4L._AC_UL800_QL65_.jpg",
      price: "Rs. 2139",
      cost: 2139,
      selling_cost: 4249,
      sellPrice: "Rs. 4249",
      discount: "(47% OFF)",
      ratings: "3.8",
      reviews: "3.1k",
      name: "Dawn Bluem",
      color: "blue",
      brand: "reebok",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/41BkOONYzqL._AC_UL800_QL65_.jpg",
      price: "Rs. 526",
      cost: 526,
      selling_cost: 749,
      sellPrice: "Rs. 749",
      discount: "(47% OFF)",
      ratings: "4.7",
      reviews: "7.2k",
      name: "Predator-Z",
      color: "green",
      brand: "tantra",
    },

  {
    type:"top",
    quantity:1,
    size:"M",
      img1: "https://m.media-amazon.com/images/I/81G9+-B8sVL._AC_UL800_QL65_.jpg",
      price: "Rs. 1339",
      cost: 1339,
      selling_cost: 2749,
      sellPrice: "Rs. 2749",
      discount: "(47% OFF)",
      ratings: "4.7",
      reviews: "12k",
      name: "Alpha Mato",
      color: "green",
      brand: "tantra",
    },
];

// Filter
// const result = clothes.filter((x) => {
// {selectd value};
// return x.cost >= 1100;
// return x.selling_cost > 20 && +x.ratings > 4;
// return x.color === "green";
// });

// Sorting

// console.log(clothes);
// console.log("_____________________________");
// console.log("_____________________________");
// console.log("_____________________________");
// clothes.sort((a, b) => a.name > b.name);
// console.log(a);

const cart = JSON.parse(localStorage.getItem("cart")) || [];
const wishlist = JSON.parse(localStorage.getItem("Add to Bag")) || [];

const show = document.getElementById("show");
// Clicking the Bag Icon
let bag = document.querySelector(".fa-bag-shopping");

bag.onclick = () => {
  location.href = "./payment.html";
};

// let val = document.querySelector(".select");
// let ans = val.value;
// console.log(ans);

const sort = () => {
  let val = document.querySelector(".arrange");
  let ans = val.value;
  console.log(ans);
  if (ans == "asc-p") {
    clothes.sort((a, b) => a.cost - b.cost);
  } else if (ans == "dsc-p") {
    clothes.sort((a, b) => b.cost - a.cost);
  } else if (ans == "str-i") {
    clothes.sort((a, b) => a.name > b.name);
  } else if (ans == "str-d") {
    clothes.sort((a, b) => a.name < b.name);
  }
  append(clothes);
  // location.reload();
};

// if (ans !== undefined) {
//   console.log(ans);
// }
const append = (clothes) => {
  show.innerHTML = null;
  clothes.map((el) => {
    // console.log(el.img1);
    let div = document.createElement("div");
    div.setAttribute("class", "box");

    let img = document.createElement("img");
    img.src = el.img1;

    let name = document.createElement("h5");
    name.innerText = el.name;

    let small_div = document.createElement("div");
    small_div.setAttribute("class", "small_div");

    let price = document.createElement("p");
    price.setAttribute("class", "cost");
    price.innerText = el.price;

    let dis_price = document.createElement("p");
    dis_price.setAttribute("class", "dis_cost");
    dis_price.style.textDecoration = "line-through";
    dis_price.innerText = el.sellPrice;

    let discount = document.createElement("p");
    discount.setAttribute("class", "discount");
    discount.innerText = el.discount;

    let btn = document.createElement("button");
    btn.innerHTML = `<i class="fa-solid fa-cart-shopping"></i>&nbsp&nbspAdd To cart`;
    btn.setAttribute("class", "btn");
    btn.style.display = "none";

    btn.onclick = () => {
      wishlist.push(el);
      cart.push(el);

      localStorage.setItem("wishlist", JSON.stringify(wishlist));
      localStorage.setItem("cart", JSON.stringify(cart));
    };

    let star = document.createElement("span");
    star.setAttribute("class", "fa-solid fa-star");

    let reviews = document.createElement("div");
    reviews.setAttribute("class", "reviews");

    reviews.innerText = `${el.ratings}`;
    reviews.append(star);

    small_div.append(reviews, price, dis_price, discount);
    div.append(img, name, small_div, btn);
    show.append(div);

    // Handling events
    div.onmouseover = () => {
      if (el.img2 !== undefined) {
        img.src = el.img2;
      }
      reviews.style.display = "none";
      btn.style.display = "block";
    };

    div.onmouseout = () => {
      img.src = el.img1;
      btn.style.display = "none";
      reviews.style.display = "block";
    };

    img.onclick = () => {
      location.href = "./women_product.html";
    };
  });
};

append(clothes);

const handleChange = (t) => {
  // console.log(t.value);
};
const handlePrice = (t) => {
  // console.log(t.value);
};
const handleColor = (t) => {
  // console.log(t.value);
};

// const move = () => {};

document.getElementById("form").onclick = function () {
  var checkboxes = document.getElementsByName("USP");
  for (var checkbox of checkboxes) {
    if (checkbox.checked) {
      // document.body.append(checkbox.value + " ");
      console.log(checkbox.value);
    }
  }
};
